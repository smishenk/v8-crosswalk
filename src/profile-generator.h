...
struct CallSite {
public:
    CallSite(CodeEntry* e = NULL, int line = v8::CpuProfileNode::kNoLineNumberInfo)
        : entry(e),
          srcLine(line){}
...
class ProfileNode {
 ...
 public:
  inline ProfileNode(ProfileTree* tree, CodeEntry* entry, int src_line = v8::CpuProfileNode::kNoLineNumberInfo);

  ProfileNode* FindChild(CallSite* callsite);
  ProfileNode* FindOrAddChild(CallSite* callsite);
...
class CpuProfile {
 public:
  ...
  void AddPath(base::TimeTicks timestamp, const Vector<CallSite>& path,
               int src_line);
